...
**Step 4 — Self-review:**
Read rules in `.antigravity/skills/code-reviewer/SKILL.md` as a checklist:
...
If running: navigate → action → verify → screenshot as evidence.
...
Input: git diff, security-reviewer SKILL.md, stack security skill, rules files.
...
Input: git diff, red-team SKILL.md, security context.
...
When spawning validation subagents (Routes B and C), the implementing agent provides instructions and file paths. Subagents read files directly from the filesystem — the implementing agent does NOT package file contents into the prompt.

**Antigravity Agent Manager:** The subagent mechanics use Antigravity's Agent Manager for spawning independent agents. The concepts (isolated context, context routing, sequencing) are identical to other tools. Adapt the instruction template below to Agent Manager's invocation syntax — validate mechanics before implementing in production projects.

**Instruction template (adapt to Agent Manager syntax):**
```
...
ALWAYS instruct the subagent to read:
  - The agent's own SKILL.md file (code-reviewer reads code-reviewer/SKILL.md, etc.)
  - .antigravity/rules/*.md (ALL rules files — cost is low, risk of omission is high)
  - GEMINI.md sections: Key Patterns, Architecture
...
NEVER instruct the subagent to read (anti-bias firewall):
  ...
  - .antigravity/logs/*.md (session history)
...
Each subagent is a fresh Agent Manager instance — isolated context, no carryover between invocations.
...
8. **Create skills on-demand** — two trigger types:
   ...
   **Proactive (predictable from context):** PRD, stack, or domain makes a skill predictable.
   ...
   - Already used in Session 0 (Steps 9, 10, 11, 11.1, and 12) for security-reviewer, Red Team/Blue Team, validator, arbitrator, and stack skills

   Skill = knowledge (HOW). Agent-role skill = judgment (WHAT to verify). Max 100 lines.
   ...
   **Frontmatter requirements:** Every new skill MUST include:
...
- GEMINI.md created with Session Protocol + Execution Protocol + Validation Orchestration
...
- security-reviewer skill created
- validator skill created (independent validation)
- arbitrator skill created (conflict resolution)
...
The code-reviewer serves dual roles: read as an inline checklist for routine tasks (Route A), and spawned as an independent subagent for logic-heavy and architecture/security tasks (Routes B and C).

**If `.antigravity/skills/code-reviewer/SKILL.md` already exists:** Do NOT overwrite. Verify it has Known Bug Patterns, Architecture Patterns, Input, Output, and BOUNDARIES sections. Add them if missing.
...
- `.antigravity/phases/project.md` Progress Log
- `.antigravity/logs/*.md`
- Sprint proposals or implementation plans
...
Universal skill created at bootstrap for ALL projects. Covers OWASP Top 10, injection prevention (SQL, XSS, prompt), auth/authz, and data protection. Stack-agnostic — covers universal principles. Stack-specific security checks are created dynamically by the proactive stack skill (Step 12) and Red Team skill (Step 10).

**If `.antigravity/skills/security-reviewer/SKILL.md` already exists:** Do NOT overwrite. Verify it has: prompt injection section, tiered security testing model reference, Section 8 delegation to stack skills/Red Team, and Input/Output/BOUNDARIES sections.
...
effort: high
description: >
  Security review skill based on OWASP Top 10 and common attack vectors.
...
Check whenever changes involve ANY of:
...
The validator is a mandatory skill created for ALL projects. It performs independent verification of the implementing agent's work — re-running build, tests, criteria checks, and mutation tests with isolated context. This is the skill that eliminates confirmation bias: the implementing agent never judges its own work on non-trivial tasks.

**If `.antigravity/skills/validator/SKILL.md` already exists:** Do NOT overwrite. Verify it has Input, Output, and BOUNDARIES sections. Add them if missing.

**If it does not exist:** Create `.antigravity/skills/validator/SKILL.md`:
...
effort: high
description: >
  Independent validation skill. Spawned via Agent Manager after implementation.
...
This skill receives (via Agent Manager prompt):
- **Git diff** — read via `git diff HEAD~1`
...
Do NOT read:
- `.antigravity/phases/project.md` Progress Log (contains implementation reasoning from previous sessions)
- `.antigravity/logs/*.md` (session history)
- Sprint proposals or implementation plans
- Any file the implementing agent wrote as part of the task explanation
...
**If `.antigravity/skills/arbitrator/SKILL.md` already exists:** Do NOT overwrite.

**If it does not exist:** Create `.antigravity/skills/arbitrator/SKILL.md`:
...
This skill is spawned ONLY when:
...
Do NOT read:
- `.antigravity/phases/project.md` Progress Log (contains implementation reasoning)
- `.antigravity/logs/*.md` (session history)
- Sprint proposals or implementation plans
- Any file the implementing agent wrote as part of the task explanation
...
### Step 14 — Configure Antigravity settings and initialize logs
...
- .antigravity/skills/arbitrator/SKILL.md ([lines] lines) ← mandatory (Step 11.1)
- .antigravity/skills/[domain]-test-patterns/SKILL.md ([lines] lines) ← if created (Step 12)
...
- validator (bootstrap — mandatory, Step 11)
- arbitrator (bootstrap — mandatory, Step 11.1)
