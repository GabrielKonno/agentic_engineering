...
| **Wrong reasoning depth** | AI uses shallow reasoning on complex financial logic, or deep reasoning on simple UI change | Four-mechanism system: agent/skill metadata for automatic depth (zero intervention), task classification for recommendations (2 seconds), tool settings edit for model switch (5 seconds), and `model:` in agent frontmatter for the DESCENDING direction (declarative, by risk class — `modules/rules/session_rules.md` → "Model by risk class"). |
| **Every agent runs at the orchestrator's model** | An agent with no declared model inherits the session's, so a mechanical extractor runs at the same tier as a money-path reviewer — invisible, because absence and decision look identical | Mechanism 4 (`modules/rules/session_rules.md` → "Model by risk class"): `model:` is MANDATORY in agent frontmatter and written explicitly (`inherit` included), so `grep -rn "^model:"` answers which model reviewed which commit. The frontmatter guard fails on a missing one, and on a SKILL that carries one (a skill never spawns, so nothing would read it). |
