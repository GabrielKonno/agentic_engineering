...
`none — no reviewer ran` when none did. When the pre-deploy gate ran this session, ALSO its line: `review receipts: N commits in range, K without receipt` or `review receipts: RED — range unreadable — deploy blocked` (validation-orchestrator → "Review receipts").]
