...
4. **UI verification (if UI files modified):** Code review or inference is NOT sufficient — browser automation is mandatory for any change that affects what the user sees.
   ...
   **Visual regression (shared component change):** If the diff modifies shared components (button, modal, nav, form, card, layout primitives) or CSS variables, ALWAYS DECLARE a **visual regression gap** in this report's Coverage Gap Declaration — NEVER attempt to spawn `visual-regression-tester` from here. This agent runs AS a subagent, and only main Claude can use the Agent tool (component-design §7: subagent depth limit). Main Claude reads the declaration, spawns the specialist, and returns its comparison. If the specialist runs and no baselines exist, it captures them and the UI result is `BASELINE-CREATED` (not ✅).
