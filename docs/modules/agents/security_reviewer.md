...
(`code_reviewer.md` resolves this the same way. The two clauses are deliberately EQUIVALENT, not
byte-identical — an earlier wording claimed identity that a `diff` refutes, since one bolds a phrase
the other leaves plain; `/audit` 2026-09-03 P-15.)
```
